...
if(!$coContent and $w != 'd'){
		print"
		<script>
		window.alert('내용을 입력하십시오.')
		history.go(-1)
		</script>
	";
}
...
<?php 
		...
		$sql = 'delete from comment_free where co_no = ' . $coNo . ' or co_order = ' . $coNo ;
...
?>
		...
		<script>
			...
			location.replace("./view_index.php?bno=<?php echo $bNo?>");
