...
<form action="comment_update.php" method="post">
	...
	<table>
		<tbody>
			<tr>
				...
				<td><input type="text" name="coId" id="coId" value=<?php echo $userid ?>></td>
			</tr>
			<tr>
				<th scope="row">
			<label for="coPassword">비밀번호</label></th>
				<td><input type="password" name="coPassword" id="coPassword" value=<?php echo $userpass ?>></td>
			</tr>
			<tr>
	...
	<div class="btnSet">
		<?php 
			if(!$userid) {
		?>
					<input type="button" value="코멘트 작성" onclick="alertFunction()">
		<?php
			} else { 
		?> 
	...
	$(document).ready(function () {
		...
		$('#commentView').delegate('.comt', 'click', function () {
			...
			if($(this).hasClass('write')) {
				...
				action = 'w';
				//ID 영역 출력
				coId = '<input type="text" name="coId" id="coId" value=<?php echo $userid ?>>';
				...
				comment += '				<td><input type="password" name="coPassword" id="coPassword" value=<?php echo $userpass ?>></td>';
				comment += '			</tr>';				
				if(action !== 'd') {
					comment += '			<tr>';
				...
				<?php if(!$userid){ ?>
				comment += '<input type="button" value="확인" onclick="alertFunction()">';
				<?php }else{ ?>
...
<html><head><script>function alertFunction() {	window.alert("로그인 후 이용하여 주십시오.");	}</script></head></html>